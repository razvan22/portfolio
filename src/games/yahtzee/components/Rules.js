...
class FullHouse extends Rule {
  evalRoll = dice => {
    const freqNumber = this.freq(dice)
    return (freqNumber.includes(2) && freqNumber.includes(3)) ? this.score : 0;
  }
  // TODO
}
...
class SmallStraight extends Rule {
  evalRoll = (dice) => {
    ...
    if (diceSet.has(2) && diceSet.has(2) && diceSet.has(4) && (diceSet.has(1) || diceSet.has(5))){
      return this.score;
    }
...
const largeStraight = new LargeStraight({ score: 40, description : "40 points for a small" });
